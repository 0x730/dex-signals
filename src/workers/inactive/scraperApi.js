...
const intervalMs =
  parseInt(process.env.TOKENSNIFFER_FETCH_INTERVAL_MS, 10) || 900000;
const maxTokens = parseInt(process.env.MAX_TOKENSNIFFER_TOKENS, 10) || 50;
...
async function fetchPageContent(url) {
  try {
    const response = await axios.get(SCRAPERAPI_BASE_URL, {
      params: {
    ...
    if (parentContainer) {
      ...
      if (riskSVG) {
        ...
        if (pathElement) {
          ...
          console.log(`Extracted path: ${pathD}`);
...
async function fetchTokenData(address) {
  const tokenUrl = `${BASE_URL}/base/${address}`; // TODO: use correct CHAIN value
 ...
 */
async function enrichTokensWithTokenSniffer() {
  logger.info('[TokenSnifferWorker] Fetching watched tokens for enrichment...');

  const tokens = await TokenModel.findTokensForTokenSnifferAnalysis(maxTokens);

  if (!tokens.length) {
    logger.info('[TokenSnifferWorker] No tokens to enrich at this time.');
    return;
  }

  logger.info(`[TokenSnifferWorker] Found ${tokens.length} tokens to enrich.`);

  for (const token of tokens) {
    const { id, address } = token;

    try {
      const { score, tokenSnifferWarning } = await fetchTokenData(address);
      if (score !== -1) {
        ...
        logger.info(
          `[TokenSnifferWorker] Updated token ID ${id} with score=${score}, warning=${tokenSnifferWarning}`
        );
      }
    } catch (err) {
      logger.error(
        `[TokenSnifferWorker] Error enriching token ID ${id}: ${err.message}`
      );
    }
  }

  logger.info('[TokenSnifferWorker] Finished TokenSniffer enrichment.');
}

/**
 * Run the Token Sniffer worker.
 */
async function runTokenSnifferWorker() {
  logger.info('[TokenSnifferWorker] Starting TokenSniffer Worker...');

  await enrichTokensWithTokenSniffer();
