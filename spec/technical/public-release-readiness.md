...
15. Runtime packages required by server code must be declared in root `package.json`; peer dependency hoisting must not be relied on for boot.
16. Server route registration must be compatible with the installed Express major version and must not crash during origin startup.
